...
\begin{abstract}
    ...
    \simeq 9.5 \, M_\odot$.  The mass function falls by at least an order of
    ...
    in our most flexible model is $m_{1\%} = 3.28^{+1.19}_{-0.13} \, M_\odot$.
    In other words, this sample of low-mass black holes does not require a mass
    gap but may permit one; observations in the currently-ongoing ``O4''
    observation run should distinguish these possibilities.  Toward higher
    masses, the mass function declines to our upper limit $m = 20 \, M_\odot$
    steeply, with power law slopes $\mathrm{d} N / \mathrm{d} m \sim
    m^{-\alpha}$, $\alpha = -4.9^{+3.1}_{-3.3}$.  We find no evidence for or
    against a secondary, sharply-peaked population of black holes around $m
    \simeq 9 \, M_\odot$ as has been suggested by several models of stellar
    evolution.
\end{abstract}
