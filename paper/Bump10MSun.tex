...
\begin{abstract}
    ...
    \simeq 9.5 \, M_\odot$, associated with merger rates $m_1 m_2 \mathrm{d} N /
    \mathrm{d} m_1 \mathrm{d} m_2 \mathrm{d} V \mathrm{d} t = 270^{+270}_{-150}
    \, \mathrm{Gpc}^{-3} \, \mathrm{yr}^{-1}$.  The mass function falls by at
    least an order of magnitude both below and above this peak.  Toward the
    lowest masses, the mass function may or may not flatten; we find that the
    $1\%$ black hole mass in our most flexible model is $m_{1\%} =
    3.28^{+1.19}_{-0.13} \, M_\odot$. In other words, this sample of low-mass
    ...
    possibilities.  Toward higher masses, the mass function declines to our
    upper limit $m = 20 \, M_\odot$ steeply, with power law slopes $\mathrm{d} N
    / \mathrm{d} m \sim m^{-\alpha}$, $\alpha = -4.9^{+3.1}_{-3.3}$.  We find no
    evidence for or against a secondary, sharply-peaked population of black
    holes around $m \simeq 9 \, M_\odot$ as has been suggested by several models
