...
\begin{abstract}
    ...
    at $m = \mpeakplgplpunits$, associated with merger rates $m_1 m_2 \mathrm{d}
    N / \mathrm{d} m_1 \mathrm{d} m_2 \mathrm{d} V \mathrm{d} t =
    \dNlogmpeakunits$.  The mass function falls by at least an order of
    magnitude both below and above this peak.  Toward the lowest masses, the
    mass function may or may not flatten; we find that the $1\%$ black hole mass
    in our most flexible model is $m_{1\%} = \monepctplgplpunits$. In other
    words, this sample of low-mass black holes does not require a mass gap but
    may permit one; observations in the currently-ongoing ``O4'' observation run
    should distinguish these possibilities.  Toward higher masses, the mass
    function declines to our upper limit $m = 20 \, M_\odot$ steeply, with power
    law slopes $\mathrm{d} N / \mathrm{d} m \sim m^{-\alpha}$, $\alpha =
...
\begin{figure}
    \includegraphics[width=\columnwidth]{figures/fg.pdf}
    \caption{\label{fig:fg} The posterior distribution for $f_g$, the fraction
    of the merger rate attributed to the Gaussian component of the power law
    plus Gaussian model at the Gaussian peak.  The solid line is the result with
    50\% selection cut, the dashed line is the result with 90\% selection cut.}
\end{figure}
