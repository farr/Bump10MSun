...
\noindent
\chcomment[id=W]{I don't think writing out the equation for the likelihood really helps here; it takes a lot more explanation than we really want to give, and appears in the papers that are cited anyway.}
...
process\chreplaced[id=W]{.  Modeling such a process subject to observations with
measurement uncertainty and selection effects is described
\citet{Mandel:2018mve,pop-vitale,popgw2,popgw3}.}{~\citep{Mandel:2018mve,pop-vitale,popgw2,popgw3},
with the hyperparameters determining the distribution of BBH parameters having
the following likelihood function:} \chdeleted[id=W]{where
$N_{det}(\vec{\lambda})$ is the number of detectable events as a function of
hyperparameters and $\left<\right>_{d_i}$ represents a sum over posterior
samples of the $ith$ event re-weighted by the prior used in parameter
estimation~(PE). $N_{det}(\vec{\lambda})$ embodies selection effects incurred
due to the imposition of a detection threshold, and is computed by re-weighting
detectable simulated events to the population distribution being
inferred~\citep{Mandel:2018mve,pop-vitale,popgw2,popgw3,Pdet1-Farr,Pdet2-essick}.}
...
the low-mass BBH population \chcomment[id=W]{I don't think this is right---there
is no upper limit to $m_1$ in the analysis, is there?}. \chreplaced[id=W,comment={$m_\mathrm{low}$ isn't fixed all the time!}]{Our canonical analysis uses}{We fix the values of
these cuts throughout all analyses to be} $(3M_{\odot}, \mhighunits,
...
\chreplaced[id=W]{To restrict to low-mass events, }{The exact implementation of
how these cuts are imposed in the analysis is described as follows.  For
selecting events, } we impose the requirement that $50\%$ of posterior samples
...
masses). \chcomment[id=W]{Are you sure about this following statement?  I don't
think it's true; sometimes for plotting we cut things off, but there isn't a
sharp cutoff in the actual mass function.} During inference, we further impose
...
systems whose only compact object components are BHs, \chcomment[id=W]{I might
say ``in contrast to'' instead of ``unlike''---it feels like we are going pretty
hard at Amanda, and I don't think that study is necessarily invalid, just that
our approach is better.} unlike other GW-based studies~\citep{Farah:2021qom}
...
\chreplaced[id=W]{term}{$N_{det}$} which corrects for selection biases in the
analysis. Note that this set of detectable simulations estimates detector
sensitivity \chreplaced[id=W]{through the}{up till} LVK's third observation run
...
\dNlogmpeakincludingnewunits. \chcomment[id=W]{Should probably say which model
is used to measure the location of and rate at the peak?}
\chcomment[id=W]{Should probably also make clear that this is the
\emph{differential} merger rate per log mass squared at the location of the
peak; and the peak has a width of about 0.1 in log mass (check this), so the
\emph{integrated} merger rate is much lower than this number $\sim 10^2$ would
suggest.} The merger rate density is found to fall off by nearly an order of
...
the}{and the} $m_{1\%}$ inference is driven by the primary component of
...
merger rate density \chcomment[id=W]{This is per log mass squared?} at the
