...
% Generated macros
\input{result_macros.tex}
% \input{result_macros_including_230529.tex}
\input{result_macros_including_230529_3.0.tex}
% Useful definitions
\newcommand{\dd}{\ensuremath{\mathrm{d}}}

\begin{document}
\title{Hiding Out at the Low End: No Gap and a Peak in the Black-Hole Mass Spectrum }
...
\begin{abstract}
    ...
    analyze the \nevts{} GWTC-3 merger events (along with
    GW230529\_181500) with at least one black hole ($3 \, M_\odot < m_1$) and chirp
	masses below those of a $20\,M_\odot$--$20\,M_\odot$ merger ($\mathcal{M} <
    ...
    \simeq (8-10 M_{\odot})$. We observe a steady decline in the merger rate to
    lower masses, but by less than an order of magnitude in total, and find that the first
    percentile of black hole masses in our most flexible model is $m_{1\%} =
    \monepctplgplpincludingnewunits$. In other words, this sample of low-mass
    black holes is not consistent with the existence of a mass gap.
...
Observations of GWs from compact binary coalescences~(CBCs) by the LIGO-Virgo-KAGRA~\citep[LVK, ][]{LIGOScientific:2014pky, VIRGO:2014yos, KAGRA:2020agh} detector network offer a novel way to probe the existence of a lower mass gap that is free of such systematic biases~\citep{Farah:2021qom,  LIGOScientific:2024elc, KAGRA:2021duu}. Selection effects in GW observations are well understood and easily modeled through the recovery of simulated sources injected into detector noise realizations~\citep{Thrane:2018qnx,Mandel:2018mve,popgw3, Tiwari:2017ndi}, which can then be used to obtain the astrophysical mass distribution of compact objects as compared to the observed distributions inferred by LMXB investigations. Previous studies have attempted this exploration by analyzing all CBC observations~(including NS containing events) using a mass distribution model that allows for a shallow gap~\citep{Farah:2021qom,  KAGRA:2021duu, LIGOScientific:2024elc}. The location, depth, and width of the gap were inferred a posteriori, and Bayesian evidences were computed for values of these parameters that correspond to the presence and absence of a lower gap. 

\cite{Farah:2021qom} have found from the second gravitational wave transient catalog~\citep{LIGOScientific:2020ibl} that the parameters controlling the gap-like feature built into their mass distribution model have a posteriori values that prefer the existence of a lower mass gap with a Bayes factor of 55.0 over its absence. \textbf{However, subsequent studies by \cite{KAGRA:2021duu} have updated this number to a lower value of 19.2 by including events from the third gravitational wave transient catalog~\citep[GWTC-3,][]{KAGRA:2021vkt} while showing that there is no significant observational preference for or against population models that explicitly account for a gap relative to ones that are agnostic of any such feature in the relevant mass range. While accounting for the higher maximum mass of rapidly spinning NSs can still lead to an observed gap in the NSBH population of GWTC-3~\citep{Ye:2022qoe}, observations of the mass gap event GW230529\_181500~\citep[a compact binary merger whose most massive component lies in the $3-5M_{\odot}$ range][]{LIGOScientific:2024elc} have reinforced the conclusion that the mass-spectrum of mergining compact objects is likely populated in the region between the heaviest NS and the lightest BH.}

%Subsequent studies by \cite{KAGRA:2021duu} and \cite{LIGOScientific:2024elc} have updated this number to lower values by including events from the third gravitational wave transient catalog~\citep[GWTC-3,][]{KAGRA:2021vkt} and the mass-gap event GW230529\_181500~\cite[a compact binary merger whose most massive component lies in the $3-5M_{\odot}$ mass range,][]{LIGOScientific:2024elc}.\cite{KAGRA:2021duu} have further reported that there is no significant observational preference in GWTC-3 for or against models that exhibit or rule out a lower mass gap as compared to a default mass distribution model, which is agnostic of any gap-like feature in the relevant mass range.
...
BH mass function~\citep[similar to the approach of X-ray binary studies,
][]{Farr:2010tu}\footnote{See Sec.~\ref{sec:methods} for a discussion regarding the robustness of percentile measurements given the size of the dataset.}.  By relying on a mass cut~\citep{Roy:2025ktr} to prevent NS components from
...
3M_{\odot}$ at $90\%$ confidence, given data from GWCT3 and GW230529\_181500. Our models
...
that of LMXB studies can therefore be attributed to \textbf{four specific possibilities. This discrepancy can either stem from the differences in
the treatment of selection effects, the systematic biases in LMXB mass measurements, the potentially distinct evolutionary
pathways of LMXBs and BH containing CBC systems, or the systematic mis-estimation
...
\label{sec:methods}
\noindent
We construct flexible population models for low-mass BBHs and analyze a sample of 25 GWTC-3 observations, both with and without GW230529\_181500, to investigate features in the mass-function and assess whether or not a gap is necessary to fit the observed dataset. The population inference and event selection methods used to carry out this exploration are delineated as follows.
% \begin{itemize}
...
approximation to the mass function of \textbf{both Galactic \citep{Farr2020,Farrow:2019xnc, Alsing2018,} and extragalactic~\citep{Landry:2021hvl, Galaudage:2020zst} binary neutron stars)}, with a peak at $\mu_\mathrm{NS}$, a width
...
We note that given the sample-size~($O(20-30)$) of events analyzed with this framework, reasonable measurement of the first percentile are obtainable. For such sample-sizes, according to the finite-sample extension of Chebychev's inequality~\citep{Saw01051984, Kaban2012-yi}, the first percentile of a distribution will be less than roughly 10 times the sample-standard-deviation away from the sample-mean. Hence, the reliability of percentile measurements from such a finite set of samples depends on that of the mean and standard deviation, which can both be reliably constrained from O(20-30) observations, within model assumptions. We show that the conclusions obtained from our $m_{1\%}$ measurements are consistent between reasonable variations of both the functional forms of our distribution functions as well as the chosen values of $m_{low}$.
...
measurement uncertainty and selection effects \textbf{as} described
...
amounts to fixing \textbf{them} to the uninformative priors used during single event
...
\begin{equation}
	m_\mathrm{low}\leq m_1 ~\&~\mathcal{M}\leq \mathcal{M}_{\mathrm{max}}.
\end{equation}
...
for each event lie within our chosen mass-cuts~\citep{Roy:2025ktr}.  Technically speaking, because
...
on the LVK sample of detected injections \citep{KAGRA:2021duu, Roy:2025ktr}, which is
...
\begin{figure}[htp]
	...
	$90\%$ of the likelihood for each event.  The \textbf{shaded region represents the part of the parameter space excluded by our}
	selection cuts, with \textbf{only the region spanned by} $m_1 > 3 \, M_\odot$ and $\mathcal{M} <
	\mchighunits$ \textbf{included in our analysis}.}
\end{figure}
\newpage
\section{Results}
\label{sec:results}
...
\begin{figure}
	...
	functions\textbf{; and dashed lines enclose the 90\% credible interval}.}
\end{figure}
...
\begin{figure}
	...
	distributions\textbf{; and dashed lines enclose the 90\% credible interval}.  At high masses, $m \gg \mu, m_b$, the mass function falls
...
year) and mass-cuts along with the mass gap event GW230529\_181500. We use single-event
...
expect to suffice since GW230529\_181500 was detected within the first two weeks of the
...
\begin{figure}
	...
	\monepctplgplpincludingnewunits$ at \textbf{90\%} credibility.  The solid lines are
...
\input{table1_content_including_230529_3.0.tex}
...
absence of a lower mass-gap for all $m_\mathrm{low}$ values chosen. \textbf{The corresponding credible intervals of $m_{1\%}$ as a function of $m_{\mathrm{low}}$ are listed in Table~\ref{tab:mlow-vs-m1}.} Both models yield more constrained and peaked posteriors for smaller values of
$m_{\mathrm{low}}$.We see the posteriors start to grow broader with increasing
$m_\mathrm{low}$. The broadening of the posterior for $m_\mathrm{low}<3M_{\odot}$ as well
...
BH mass function for $m_\mathrm{low}>2.7M_{\odot}$, at which point
the $m_{1\%}$ inference is driven by the primary component of
GW230529\_181500. For  $m_\mathrm{low}>3.25M_{\odot}$, posterior samples of GW230529\_181500 start
...
%\input{table1_content.tex}

%\clearpage
%\subsection{Including GW230529}


\textbf{We note here that additional data from the complete fourth gravitational wave transient catalog \citep[GWTC-4, ][which was publicly unavailable at the time of submission]{LIGOScientific:2025slb} are in agreement with our our conclusions obtained from the much smaller dataset that only comprises GWTC-3 and the mass gap event GW230529\_181500. In particular, population analyses with the complete fourth catalog also find diminished support for a dearth of BHs in the $3-5M_{\odot}$ range and a shallow rise in the mass-spectrum upto a peak near $\sim10M_{\odot}$~\citep{LIGOScientific:2025pvj}. This is to be expected since GW230529\_181500 is the only event in GWTC-4 that contributes significantly to the inferred shapes of the mass spectrum below $10M_{\odot}$. However, the fall off at masses above the $10M_{\odot}$ peak is much sharper in GWTC-4, which is followed by a nearly flat plateau upto $\sim20M_{\odot}$\citep{LIGOScientific:2025pvj}. Hence, even though a reanalysis with GWTC-4 is not expected to change our key results which mostly concerns the lower end of the mass distribution at and below the $10M_{\odot}$ peak, it would likely necessitate more flexible models for the BBH part of the mass-spectrum, i.e., between the $10M_{\odot}$ peak and our high-mass cut of $\mathcal{M}\leq20M_{\odot}$. Such an investigation is currently ongoing.}
...
In accordance to existing studies~\citep[e.g.,][]{KAGRA:2021duu, LIGOScientific:2024elc, Tiwari:2021yvr, Farah:2023vsc, Ray:2023upk, Sadiq:2023zee, Edelman:2022ydv, Callister:2023tgi, Ray:2023upk, Ray:2024hos, Heinzel:2024hva}, we also recover an excess
...
\textbf{The presence of both a strong peak and a shallow drop in merger rate density towards the lower end of the BH mass spectrum has interesting astrophysical implications, particularly with regards to the roles played by core collapse, natal kicks, and treatments of binary stellar evolution in shaping the resultant population of compact binary mergers~\citep{Belczynski:2017gds, Giacobbo:2018etu,Schneider:2020vvh, vanSon:2022myr}. In particular, several} models of compact binary formation, such as isolated stellar binaries undergoing stable
...
\begin{acknowledgments}
    We thank Jeff Andrews for comments on an early version of this work. This work was initiated at the Aspen Center for Physics, which is supported by National Science Foundation grant PHY-2210452. This
...
\software{\texttt{Makie.jl} \citep{DanischKrumbiegel2021}; \texttt{Turing.jl} \citep{ge2018t}; \texttt{zenodo-get} \citep{Volgyes2020}}
